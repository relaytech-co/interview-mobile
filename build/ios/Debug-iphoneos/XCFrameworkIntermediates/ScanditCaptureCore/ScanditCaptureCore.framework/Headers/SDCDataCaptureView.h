...
SDC_EXTERN BOOL SDCLogoStyleFromJSONString(NSString *_Nonnull JSONString,
                                           SDCLogoStyle *_Nonnull style) DEPRECATED_MSG_ATTRIBUTE("This function will be removed in the future");
 ...
 */
@property (nonatomic, weak, nullable) SDCDataCaptureContext *context;
/**
 * Added in version 6.0.0
 *
 ...
 *
 *   • SDCTorchSwitchControl will be placed to the top-left.
 *
 *   • SDCCameraSwitchControl will be placed to the top-right.
 *
 *   • SDCZoomSwitchControl will be placed to the bottom-right.
 *
 *   • SDCMacroModeControl will be placed to the top-right.
 *
 *   • SDCLinearControlGroup will be placed to the bottom-right.
 *
 * Only one control per type can be added at any point in time, trying to add a second instance of the same control type will result in the first one being removed.
 * Adding a control at an anchor already taken will replace the previous control there. If you need to place multiple controls at the same anchor, consider using SDCLinearControlGroup.
 */
- (void)addControl:(nonnull id<SDCControl>)control NS_SWIFT_NAME(addControl(_:));
/**
 * Added in version 6.14.0
 *
 * Adds the control to the data capture view at the specified anchor and with the given offset.
 * Only one control per type can be added at any point in time, trying to add a second instance of the same control type will result in the first one being removed.
 * Adding a control at an anchor already taken will replace the previous control there.
 * The offset x will move the control away from the border of the data capture view horizontally, which means that controls anchored on the left will move to the right and vice-versa. Controls anchored in the center horizontally will also move to the right.
 * The offset y will move the control away from the border of the data capture view vertically, which means that controls anchored on the top will move downward and controls anchored to the bottom will move upward. Controls anchored in the center vertically will also move upward.
 *
 * Adds the control to the data capture view at the specified anchor and with the given offset.
 * Only one control per type can be added at any point in time, trying to add a second instance of the same control type will result in the first one being removed.
 * Adding a control at an anchor already taken will replace the previous control there. If you need to place multiple controls at the same anchor, consider using SDCLinearControlGroup.
 ...
 * The offset y will move the control away from the border of the data capture view vertically, which means that controls anchored on the top will move downward and controls anchored to the bottom will move upward. Controls anchored in the center vertically will also move upward.
 */
- (void)addControl:(nonnull id<SDCControl>)control
            anchor:(SDCAnchor)anchor
            offset:(SDCPointWithUnit)offset NS_SWIFT_NAME(addControl(_:anchor:offset:));
/**
 * Added in version 6.0.0
 *
